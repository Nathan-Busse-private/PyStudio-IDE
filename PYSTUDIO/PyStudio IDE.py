...
os.system('cmd /c "pip install tKinter"') # Automatically installs required packages for IDE to work.
os.system('cmd /c "pip install webbrowser"') # Automatically installs required packages for IDE to work.
# os.system('cmd /c "pip install SciKit-learn"')

os.system('cmd /c mkdir Python')


root = Tk ()
root.title ( 'PyStudio IDE' )
#root.iconbitmap ( 'PyStudio.ico' )
...
def new_file():
    ...
    # Update status bars
    root.title ( 'New File - PyStudio IDE' )
...
def save_file():
    ...
    if open_status_name:
        ...
        name = name.replace ( "C:/gui/" , "" )
        root.title ( f'{name} - PyStudio IDE!' )
    else:
        save_as_file ()
...
def night_on():
    ...
    #developer_menu.config ( bg=main_color , fg=text_color )
...
# Developers
# Add Options Menu
developer_menu = Menu ( my_menu , tearoff=False )
my_menu.add_cascade ( label="Developers" , menu=developer_menu )
developer_menu.add_command ( label="Natham Busse"  )
